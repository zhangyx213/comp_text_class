...
#wells comment
#better to spell out the words rather than using # for shorthand for numbers
...
```{r}
#needed to filter the NA values
vas%>%
...
#Wells comment - not correct. See revision
```{r}
vas%>%
  group_by(publication_location) %>%
  summarize(
...
#Wells comment: Please write down the prompt and the results when you use AI for coding assistance so I can help you. This code failed below
...
#Wells comment: The headline needs revision. What is this chart measuring? The amount of vascectomy news coverage since Roe v Wade. Make your headlines engaging and descriptive. Also, do not use # interchangably for "number." Write it out. And there's a typo in the headline
```
...
#Wells comment - Im not sure that will work. The data may be missing from the source
To reduce missing values in future data collection, I can standardize fields such as “bylines,” making it a required element when I'm trying to download data from the Nexius Uni. 
#wells comment - this will worjk
Also, I can filter out the government statements and press releases by just selecting "newspaper" in the type of the publications to reduce the NAs in bylines columns. 
